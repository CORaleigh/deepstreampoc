...
def camera_counts(js, csv=False):
...
#    counts = json.dumps(counts)
    ...
    if csv:
        export_fields = [
            ...
            counts['timestamp'],
            counts['count'],
            counts['geometry']['coordinates'][0],
            counts['geometry']['coordinates'][1],
            counts['geometry']['crs']['properties']['code']
            #counts['geometry']
        ]

        counts = ','.join([str(i) for i in export_fields])

    return counts


if __name__ == '__main__':
    descrip = "Parse messages from Deepstream for use with GeoEvent"
...
#        action='store_true')
    ...
    print(f"Publishing to {cfg.get('out-count-topic')}")
...
#        print("this msg is printed for every msg in consumer")
        ...
        try:
             ...
             #counts = json.dumps(counts)
             #counts = ','.join(['literally','just','a','string'])
        except:
            counts = None
            pass
